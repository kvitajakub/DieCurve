...
jQuery.fn.colourPicker = function (conf) {
	...
	return this.each(function () {
		...
		var icon	= jQuery('<a href="#"><img src="' + config.ico + '" alt="' + config.openTxt + '" class="barvaHraceImg" /></a>').insertAfter(select);
		var input	= jQuery('<input type="text" name="' + select.attr('name') + '" value="' + select.val() + '" size="4" class="barvaHrace" readonly/>').insertAfter(select);
		var loc		= '';
      ...
      var barvaFunc= function () {
		...
		icon.click(barvaFunc);
		input.click(barvaFunc);
	});
};
